...
let MatPanelButton = class MatPanelButton extends MatButton {
    _init(params = {}) {
        ...
        const panelSizeSignal = Me$O.msThemeManager.connect('panel-size-changed', () => {
            this.queue_relayout();
        });
        ...
        this.connect('destroy', () => {
            Me$O.msThemeManager.disconnect(panelSizeSignal);
    ...
    vfunc_get_preferred_width(_forHeight) {
        return [
            Me$O.msThemeManager.getPanelSize(this.monitorIndex),
            Me$O.msThemeManager.getPanelSize(this.monitorIndex),
        ];
    }
        ...
        this.signals.forEach((signal) => {
            if (signal.from) {
                try {
                    ...
                    Me$N.log(`Failed to disconnect signal ${signal.id} from ${signal.from} ${signal.from.constructor.name} ${signal.from.toString()}  `);
                }
            }
        });
    }
...
const getSettings = (key) => {
    const schema = Gio$d.SettingsSchemaSource.new_from_directory(Me$M.dir.get_child('schemas').get_path(), Gio$d.SettingsSchemaSource.get_default(), false).lookup(Me$M.metadata[key], true);
    if (schema !== null) {
...
class MsThemeManager extends MsManager {
    constructor() {
        ...
        this.themeFile = Gio$c.file_new_for_path(`${GLib$k.get_user_cache_dir()}/${Me$L.uuid}-theme.css`);
    ...
    async buildThemeStylesheetToFile(file) {
        const originThemeFile = Gio$c.file_new_for_path(`${Me$L.path}/style-${this.themeValue}-theme.css`);
...
let MsStatusArea = class MsStatusArea extends Clutter$n.Actor {
        ...
        this.gnomeShellPanel.menuManager._menus.forEach((menuData) => {
            if (menuData.menu._boxPointer) {
                ...
                menuData.menu._boxPointer._calculateArrowSide = function () {
                    return Me$J.msThemeManager.verticalPanelPosition ===
    ...
    disable() {
        Me$J.logFocus('disable statusArea');
...
let MsDateMenuBox = class MsDateMenuBox extends St$g.Widget {
    _init(dateMenu) {
        ...
        this.notificationIcon = new St$g.Icon({
            gicon: Gio$b.icon_new_for_string(`${Me$J.path}/assets/icons/bell-symbolic.svg`),
        });
        this.notificationIconRing = new St$g.Icon({
            style_class: 'primary',
            gicon: Gio$b.icon_new_for_string(`${Me$J.path}/assets/icons/bell-ring-symbolic.svg`),
        });
        this.dndIcon = new St$g.Icon({
            gicon: Gio$b.icon_new_for_string(`${Me$J.path}/assets/icons/bell-off-symbolic.svg`),
        });
        ...
        if (Me$J.msThemeManager.clockHorizontal) {
            this.add_child(this.iconDisplay);
        }
        else {
            this.add_child(this.clockLabel);
        }
        Me$J.msThemeManager.connect('clock-horizontal-changed', () => {
            if (Me$J.msThemeManager.clockHorizontal) {
...
let MsWindow = class MsWindow extends Clutter$l.Actor {
    ...
    updateMetaWindowPositionAndSize() {
        ...
        let shouldBeMaximizedHorizontally = this._metaWindow.maximized_horizontally;
    ...
    async onMetaWindowsChanged() {
        if (this.metaWindow) {
            this.metaWindowIdentifier =
                Me$G.msWindowManager.buildMetaWindowIdentifier(this.metaWindow);
            this.reactive = false;
...
const dragData = null;
let isTileableItem = (obj) => {
    return obj instanceof TileableItem;
};
let isIconTaskBarItem = (obj) => {
    return obj instanceof IconTaskBarItem;
};
let isTileableItemOrIconTaskBarItem = (obj) => {
...
let TaskBar = class TaskBar extends St$c.Widget {
    _init(msWorkspace, panelMenuManager) {
        ...
        this.msWorkspaceSignals = [
            msWorkspace.connect('tileableList-changed', (_, newTileableList, oldTileableList) => {
    ...
    onTileableListChange(newTileableList, oldTileableList) {
        const tileableToRemove = oldTileableList.filter((tileable) => !newTileableList.includes(tileable));
        const tileableToAdd = newTileableList.filter((tileable) => !oldTileableList.includes(tileable));
...
let TaskBarItem = class TaskBarItem extends MatButton {
    ...
    vfunc_parent_set() {
        ...
        if (actor.is_mapped()) {
            this.monitor = Main$h.layoutManager.findMonitorForActor(actor);
        }
    }
    vfunc_get_preferred_height(_forWidth) {
...
let PanelContent = class PanelContent extends St$8.BoxLayout {
    _init() {
        ...
        this.searchButton = new MatPanelButton({
            ...
            primary: true,
            height: Math.max(48, Me$z.msThemeManager.getPanelSize(Main$e.layoutManager.primaryIndex)),
        });
        ...
        Me$z.msThemeManager.connect('panel-size-changed', () => {
            ...
            this.searchButton.height = Math.max(48, Me$z.msThemeManager.getPanelSize(Main$e.layoutManager.primaryIndex));
            this.queue_relayout();
        });
        this.setIcon('search');
    }
...
let SearchContent = class SearchContent extends St$8.BoxLayout {
    _init() {
        ...
        Me$z.msThemeManager.connect('panel-size-changed', () => {
            this.searchEntryBin.height = Math.max(48, Me$z.msThemeManager.getPanelSize(Main$e.layoutManager.primaryIndex));
            this.queue_relayout();
        });
    ...
    checkUnderThePointer() {
        ...
        if (monitor.index !== msWorkspace.monitor.index) {
            ...
            if (monitor === Main$7.layoutManager.primaryMonitor) {
                newMsWorkspace =
                    Me$d.msWorkspaceManager.getActivePrimaryMsWorkspace();
            }
            else {
                newMsWorkspace =
                    Me$d.msWorkspaceManager.getMsWorkspacesOfMonitorIndex(monitor.index)[0];
            }
